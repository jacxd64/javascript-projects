...
function askForNumber() {
  ...
  while(!numPass) {
    ...
    if (numMeals > 6 || numMeals < 1 || isNaN(numMeals)) {
      console.log("Value is out of range, try again.\n")
    } else {
      numPass = true;
    }
  }
...
function generatePassword(string1, string2) {
  ...
  for (let i = 0; i < string1.length; i++){
    codeArr.push(string1[i])
    codeArr.push(string2[i])
  }
  code = codeArr.join('');
...
function runProgram() {
  ...
  /// We've started with the number 2 for now. Does your solution still work if you change this value? ///
   ...
   let meals = mealAssembly(protein, grains, veggies, beverages, desserts, 7);
   console.log(meals);
